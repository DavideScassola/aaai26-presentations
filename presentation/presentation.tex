...
\begin{frame}[fragile]{Why is generating relational data hard?}
  ...
  \begin{itemize}
    \item Modeling relationships across tables: statistical dependencies of any record with any other record directly or indirectly connected to it through foreign-key relationships
    %\item Generating realistic foreign key structures
    \item Adapting to complex foreign key structures
    \item Tabular data generation: heterogeneous data types, complex marginals and dependencies
  \end{itemize}
...
\begin{frame}[fragile]{Relational Data as a Graph}
  ...
  \begin{itemize}
    ...
    \item i.i.d samples? Connected components of the graph.
  \end{itemize}
...
\begin{frame}[fragile]{Our Approach}
 Graph-conditional generation with flow matching:
  ...
  \begin{enumerate}
    ...
    \item Use flow matching (diffusion-like generative model) for generating whole connected components $(X, G)$ by "filling" the empty foreign key graph with $G$ content $X$
  \end{enumerate}
...
\begin{frame}[fragile]{Results: Fidelity}
  \begin{itemize}
    \item We use the SyntheRela benchmark library \citep{hudovernik2024benchmarking}.
    ...
    \item Records are enriched with aggregated informations from connected records
...
\begin{frame}[fragile]{Results: Privacy and Efficiency}
  \begin{itemize}
    \item \textbf{Privacy}: DCR (Distance to Closest Record) analysis does not highlight privacy leaks.
...
\begin{frame}[fragile]{Our Contribution}
  ...
  \begin{itemize}
    \item Model for whole connected components (no independence assumptions) with flow matching + GNN
    \item Generalization through modular denoiser, GNN embedding size bottleneck, controlling “within sample” generalization
  \end{itemize}

  \textbf{Flexibility}
  \begin{itemize}
    \item GNN denoiser supports any kind of graph.
  \end{itemize}

  \textbf{Scalability}
  \begin{itemize}
    \item Flow matching scales with large dimensionalities.
    \item Avoid dealing with dense adjacency matrix.
    \item GNN scales since number of edges is proportional to number of nodes.
  \end{itemize}
